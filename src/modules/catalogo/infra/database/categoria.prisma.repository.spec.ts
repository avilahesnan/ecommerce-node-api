...
describe('Repositório Prisma: Categoria', () => {
    ...
    describe('Recuperar Categoria por ID', () => {
        ...
        test('Deve Recuperar Uma Categoria por UUID', async () => {
            ...
            const categoriaRecuperada = await categoriaRepositorio.recoverByUuid(categoria.id)

            expect(categoriaRecuperada)
                .toEqual(categoria)

            expect(prismaMock.categoria.findUnique)
    ...
    describe('Recuperar Todas as Categorias', () => {
        ...
        test('Deve Recuperar Todas as Categorias Sem Execeção', async () => {
            ...
            const categorias: Array<Categoria> = listaCategoriasPrisma.map((categoria) => CategoriaMap.fromPrismaModeltoDomain(categoria))

            const todasCategoriasRecuperadas = await categoriaRepositorio.recoverAll()

            expect(todasCategoriasRecuperadas)
                .toStrictEqual(categorias)
            
            expect(prismaMock.categoria.findMany)
                .toHaveBeenCalledTimes(1)
        })
    })
    ...
    describe('Existe Categoria', () => {
        ...
        test('Deve Verificar se Existe Uma Determinada Categoria por UUID', async () => {
            ...
            const existeCategoria = await categoriaRepositorio.exists(categoriaPrisma.id)

            expect(existeCategoria)
                .toBeTruthy()
        })
    })
    ...
    describe('Inserir Categoria', () => {
        ...
        test('Deve Inserir Uma Categoria', async () => {
            ...
            const categoriaInserida = await categoriaRepositorio.insert(categoria)

            expect(categoriaInserida)
                .toStrictEqual(categoria)

            expect(prismaMock.categoria.create)
                .toHaveBeenCalledTimes(1)
    ...
    describe('Alterar Categoria', () => {
        ...
        test('Deve Atualizar Uma Categoria', async () => {
            ...
            const categoriaAtualizada = await categoriaRepositorio.update(categoria.id, categoria)

            expect(categoriaAtualizada)
                .toBeTruthy()

            expect(prismaMock.categoria.update)
                .toHaveBeenCalledTimes(1)
    ...
    describe('Deletar Categoria', () => {
        ...
        test('Deve Deletar Uma Categoria por UUID', async () => {
            ...
            const categoriaDeletada = await categoriaRepositorio.delete(categoria.id)

            expect(categoriaDeletada)
                .toBeTruthy()

            expect(prismaMock.categoria.delete)
                .toHaveBeenCalledTimes(1)
